...
//    val cluster by lazy { js("(require('cluster'))") }
    ...
    override suspend fun Worker(): WorkerChannel {
        ...
        val bootstrapUrl = URL.createObjectURL(
            Blob(
                arrayOf(
                    """
        ...
        """.trimIndent()
                ), BlobPropertyBag(type = "text/javascript")
            )
        )
        ...
        if (declaredMain != null) {
            jsWorker.postMessage(
                jsObject("type" to "import", "urls" to arrayOf(requireJsScript.src))
            )
            jsWorker.postMessage(
                jsObject("type" to "eval", "script" to "requirejs.config({ baseUrl: ${baseUrl.quote()} });")
            )
            jsWorker.postMessage(
                jsObject(
                    "type" to "eval",
                    "script" to "require([${declaredMain.quote()}], function(module) { }, function(e) { console.error(e); });"
                )
            )
        } else {
            jsWorker.postMessage(
                jsObject(
                    "type" to "import",
                    "urls" to scripts.filter { !it.src.isNullOrBlank() }.map { it.src }.toTypedArray()
                )
            )
        }
        ...
        val worker = object : WorkerChannel {
            override suspend fun send(message: WorkerMessage) {
                jsWorker.postMessage(
                    jsObject("type" to "message", "kind" to message.type, "args" to message.args)
                )
            }
