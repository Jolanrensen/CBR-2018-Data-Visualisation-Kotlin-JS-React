...
fun JobDescriptor.getName(): String = WorkerInterfaceImplBrowser.getClassName(this::class)
...
class Jobs private constructor(val worker: WorkerChannel?) {
    ...
    suspend fun execute(job: JobDescriptor, args: Array<Any?>): Array<Any?> = if (worker != null) {
        val id = lastId++
    ...
    companion object {
        suspend operator fun invoke(scope: CoroutineScope): Jobs {
            val worker = if (WorkerInterfaceImplBrowser.WorkerIsAvailable()) Worker() else null
...
fun JobsMainSync(register: suspend JobsRegister.() -> Unit, main: suspend CoroutineScope.() -> Unit, coroutineContext: CoroutineContext = EmptyCoroutineContext) {
    WorkerInterfaceImplBrowser.runEntry(coroutineContext) {
