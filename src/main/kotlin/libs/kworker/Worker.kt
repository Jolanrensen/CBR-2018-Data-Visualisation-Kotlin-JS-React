...
class WorkerMessage private constructor(
    ...
    companion object {
        operator fun invoke(type: String, vararg args: Any?): WorkerMessage {
            for ((index, arg) in args.withIndex()) when (arg) {
                ...
                else -> Unit//error("WorkerMessage args can only have null, String, Int, Double and ByteArray as arguments, but found '$arg' at index=$index.")
            }
...
abstract class WorkerInterface {
    ...
    open suspend fun WorkerFork(
        worker: suspend WorkerChannel.() -> Unit,
...
//val WorkerInterfaceImpl = WorkerInterfaceImplBrowser

fun suspendTest(callback: suspend CoroutineScope.() -> Unit): Unit =
    WorkerInterfaceImplBrowser.suspendTest { callback(CoroutineScope(coroutineContext)) }

suspend fun getWorkerId() = WorkerInterfaceImplBrowser.getWorkerId()
suspend fun Worker(): WorkerChannel = WorkerInterfaceImplBrowser.Worker()
fun WorkerIsAvailable(): Boolean = WorkerInterfaceImplBrowser.WorkerIsAvailable()
suspend fun WorkerFork(worker: suspend WorkerChannel.() -> Unit, main: suspend CoroutineScope.() -> Unit): Unit =
    WorkerInterfaceImplBrowser.WorkerFork(worker, main)
